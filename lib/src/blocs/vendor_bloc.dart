...
class VendorBloc {
  ...
  pickImage() async {
   ...
   // var permissionStatus = await photos.status;
      ...
      if (image != null) {
        ...
        if (properties.height! > properties.width!.toInt()) {
          ...
          croppedFile = await FlutterNativeImage.cropImage(image.path, 0,
              yoffset.toInt(), properties.width!.toInt(), properties.width!.toInt());
          ...
          croppedFile = await FlutterNativeImage.cropImage(image.path,
              xoffset.toInt(), 0, properties.height!.toInt(), properties.height!.toInt());
        } else {
